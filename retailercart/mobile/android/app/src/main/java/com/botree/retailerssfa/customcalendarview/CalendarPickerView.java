package com.botree.retailerssfa.customcalendarview;
...
public class CalendarPickerView extends ListView {
  ...
  public CalendarPickerView(Context context, AttributeSet attrs) {
    ...
    headerTextColor = a.getColor(R.styleable.CalendarPickerView_tsquare_headerTextColor,
        ContextCompat.getColor(context, R.color.calendar_text_active));
    ...
    while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
      ...
      for (int c = 0; c < 7; c++) {
        ...
        if (selectedCals.size() > 1) {
          ...
          } else if (sameDate(maxDates(selectedCals), cal)) {
  ...
  private static Calendar maxDates(List<Calendar> selectedCals) {
    Collections.sort(selectedCals);
    return selectedCals.get(selectedCals.size() - 1);
  }
